    ...
    <div className="flex items-start gap-4 relative z-10">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm font-medium text-slate-200 leading-relaxed line-clamp-3 italic">
          "{String(profile.diagnosisSummary || '')}"
        </p>
      </div>
    </div>
...
const ClinicalInsights: React.FC<{ profile: PatientProfile }> = ({ profile }) => {
  const recommendationText = typeof profile.latestInsight?.recommendation === 'string' 
    ? profile.latestInsight.recommendation 
    : "Analiz ediliyor: İyileşme periyodu stabil, yüklenme fazına geçiş için uygun biyomekanik altyapı mevcut.";
...
const TreatmentRow: React.FC<{ exercise: Exercise; onClick: () => void; index: number }> = ({ exercise, onClick, index }) => {
  const titleText = String(exercise.titleTr || exercise.title || '');
...
export const Dashboard: React.FC<DashboardProps> = ({ profile, onExerciseSelect }) => {
  const phaseText = typeof profile.latestInsight?.phaseName === 'string' ? profile.latestInsight.phaseName : 'Faz 1 (Akut)';
      ...
      <div className="lg:col-span-8 space-y-6">
        <div className="flex justify-between items-end px-2">
          <div>
            ...
            <p className="text-[10px] font-medium text-slate-500 uppercase tracking-widest mt-1">Rehabilitasyon Fazı: {phaseText}</p>
          </div>
